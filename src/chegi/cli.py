...
def gitignore(
    ...
    Prompts the user to select a programming language using an interactive menu, 
    combines the specific language template with global OS/IDE rules, and generates 
    a .gitignore file in the specified directory. Optionally, it can automatically 
    ...
    # 1. Interactive selection logic using questionary
    options = list(TEMPLATES.keys())
    languages = [opt for opt in options if opt != "Global (OS/IDE)"]
    
    selected_lang = questionary.select(
        "Select a language for the .gitignore file:",
        choices=languages,
        use_indicator=True
    ).ask()
    
    # Handle user cancellation (e.g. pressing Ctrl+C)
    if not selected_lang:
        ui.console.print("[bold red]Operation cancelled.[/bold red]")
        raise typer.Exit(1)
