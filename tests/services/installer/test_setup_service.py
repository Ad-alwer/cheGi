...
def test_sort_dependencies(setup_service):
    """Test the dependency sorting logic. Pure business logic test without mocks!"""
    unsorted_tools = [
    ...
    # Expected order: BaseTool -> AppA -> AppC
...
def test_execute_installations_skips_missing_deps(mock_run_cmd, setup_service):
    ...
    # 'ToolA' is NOT in setup_service.installed_tools
    setup_service.installed_tools = set()
    
    setup_service._execute_installations(tools_to_install, session_mirrors={})
    
    # ToolB should be skipped, so run_custom_command should not be called
